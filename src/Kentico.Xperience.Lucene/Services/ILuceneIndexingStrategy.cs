...
public interface ILuceneIndexingStrategy
{
    ...
    /// <param name="language">The language in which the site is indexed.</param>
    /// <returns>The value that will be indexed in Lucene.</returns>
    Task<object?> OnIndexingProperty(IWebPageContentQueryDataContainer pageContentContainer, string propertyName, string usedColumn, object? foundValue, string language);

    /// <summary>
    /// Called when indexing a search model. Enables overriding of multiple fields with custom data.
    /// </summary>
    /// <param name="lucenePageItem">The <see cref="IWebPageContentQueryDataContainer"/> currently being indexed.</param>
    ...
    Task<LuceneSearchModel> OnIndexingNode(IndexedItemModel lucenePageItem, LuceneSearchModel model);
