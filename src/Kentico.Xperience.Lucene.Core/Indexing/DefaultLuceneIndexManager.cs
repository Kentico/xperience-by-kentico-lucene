...
internal class DefaultLuceneIndexManager : ILuceneIndexManager
{
    ...
    public IEnumerable<LuceneIndex> GetAllIndices()
    {
        ...
        {
            var luceneIndices = storageService.GetAllIndexData().Select(x => new LuceneIndex(x, StrategyStorage.Strategies, AnalyzerStorage.Analyzers, AnalyzerStorage.AnalyzerLuceneVersion));
